...
class ScheduleScrapeVC: UIViewController {
    
    @IBOutlet weak var back: UIViewX!
...
//        self.loadingView.isHidden = true
    ...
    func transitionToColors() {
        ...
        UIView.transition(with: back, duration: 1.5, options: [.transitionCrossDissolve], animations: {
            self.back.firstColor = self.colorArray[self.currentColorArrayIndex].color1
            self.back.secondColor = self.colorArray[self.currentColorArrayIndex].color2
        }) { (success) in
            self.transitionToColors()
        }
    }
